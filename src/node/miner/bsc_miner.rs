    ...
    Provider: HeaderProvider + BlockNumReader + Send + Sync + Clone + 'static,
{
    ...
    async fn submit_payload(
        ...
        // Check double sign via the cache shared with the BidBlock path
        // (`try_submit_winning_bid_block` in payload.rs) — a validator must not sign two
        // different blocks at the same height on the same parent, regardless of which path
        // produced either one.
    ...
    Pool: TransactionPool<Transaction: PoolTransaction<Consensus = TransactionSigned>> + 'static,
{
    ...
    /// Pop an admitted BEP-675 BidBlock (from `mev_sendBidBlock`) and verify/blind-sign/seal it
    /// into the simulator's best-bid-block store (go-bsc `newBidBlockLoop` → `AddBidBlock`).
