    ...
    fn on_new_block_hashes(&mut self, hashes: NewBlockHashes, peer_id: PeerId) {
        ...
        for hash_number in hash_numbers {
            ...
            // Prefer the announcing peer; if it doesn't have bsc extension, fallback to any bsc peer.
            {
                ...
                let target_peer =
                    if crate::node::network::bsc_protocol::registry::has_registered_peer(
                        announcing_peer,
                    ) {
                ...
                if let Some(bsc_peer) = target_peer {
                    ...
                    tokio::spawn(async move {
                        ...
                        let req_timeout =
                            Duration::from_millis(DOWNLOAD_COOLDOWN_DURATION_MS as u64);
            ...
            let download_fut = Box::pin(async move {
                match engine
                    .fork_choice_updated(forkchoice_state, None, EngineApiMessageVersion::V1)
                    .await
                {
                    Ok(result) => {
                        tracing::debug!(
