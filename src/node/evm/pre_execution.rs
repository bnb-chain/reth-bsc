...
use crate::{
    consensus::parlia::{
        ...
        util::{debug_header, is_breathe_block},
        vote::MAX_ATTESTATION_EXTRA_LENGTH,
        Snapshot, VoteAddress, DIFF_INTURN, DIFF_NOTURN,
    },
    node::evm::{
        error::{BscBlockExecutionError, BscBlockValidationError},
        util::HEADER_CACHE_READER,
    },
    system_contracts::feynman_fork::ValidatorElectionInfo,
};
...
use reth_evm::{
    eth::receipt_builder::ReceiptBuilder, execute::BlockExecutionError, Evm, FromRecoveredTx,
    FromTxWithEncoded, IntoTxEnv,
};
    ...
    R::Transaction: Into<TransactionSigned>,
{
    ...
    pub(crate) fn eth_call(
        ...
        // etc.), these side effects are acceptable because the contracts don't use BASEFEE
        // in view functions. If future contracts depend on BASEFEE, this approach would
        // need revisiting.
        let tx_env = BscTxEnv {
            base: TxEnv {
