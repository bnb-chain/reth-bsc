...
impl Snapshot {
    ...
    pub fn new(
        ...
        if let Some(vote_addrs) = vote_addrs {
            ...
            // Pre-Luban: vote addresses are unknown, but index should still be correct
            for (i, v) in validators.iter().enumerate() {
                let info = ValidatorInfo {
                    index: i as u64 + 1,               // 1-based index
                    vote_addr: VoteAddress::default(), // All zeros
                };
                validators_map.insert(*v, info);
            }
        }
            ...
            if let Some(vote_addrs) = vote_addrs {
                ...
                for (i, v) in new_validators.iter().enumerate() {
                    validators_map.insert(
                        *v,
                        ValidatorInfo { index: i as u64 + 1, vote_addr: vote_addrs[i] },
                    );
                }
            } else {
                // Pre-Luban: vote addresses are unknown, but index should still be correct
