...
fn test_is_inturn_check() {
    ...
    for validator in &validators {
        if *validator != inturn {
            assert!(!snapshot.is_inturn(*validator), "Non-inturn validator should not be marked as inturn");
        }
    }
}
...
fn test_validator_map_completeness() {
    ...
    for validator in &validators {
        assert!(snapshot.validators_map.contains_key(validator), 
            "Validator {:?} should be in validators_map", validator);
    }
...
fn test_epoch_boundary() {
    ...
    assert_eq!(epoch_num.is_multiple_of(epoch_num), true, "Epoch boundary should be at epoch_num");
    assert_eq!((epoch_num * 2).is_multiple_of(epoch_num), true, "Double epoch should be boundary");
    assert_eq!((epoch_num + 1).is_multiple_of(epoch_num), false, "Non-boundary block");
    assert_eq!((epoch_num - 1).is_multiple_of(epoch_num), false, "Block before boundary");
}
...
fn test_validator_count_calculations() {
    ...
    for (total, expected_quorum) in test_cases {
        ...
        assert_eq!(quorum, expected_quorum, 
            "Quorum for {} validators should be {}, got {}", total, expected_quorum, quorum);
    }
}
...
mod validator_cache_tests {
    use super::*;
    use crate::node::evm::pre_execution::{VALIDATOR_CACHE, TURN_LENGTH_CACHE};
    ...
    fn test_cache_multiple_entries() {
        let entries: Vec<(B256, (Vec<Address>, Vec<VoteAddress>))> = (0..5)
            .map(|_| {
