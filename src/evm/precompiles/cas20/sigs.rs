...
// One table defines both metric labels and their fixed handle indices.
macro_rules! selector_labels {
    ($($selector:ident => $name:literal),* $(,)?) => {
        #[allow(non_camel_case_types)]
        enum SelectorIndex { $($selector,)* Unknown, Short }
        pub(crate) const SELECTOR_NAMES: &[&str] = &[$($name,)* "unknown", "short"];
        pub(crate) fn selector_index(data: &[u8]) -> usize {
            let Some(selector) = data.get(..4) else { return SelectorIndex::Short as usize };
            match selector.try_into().unwrap() {
                $($selector => SelectorIndex::$selector as usize,)*
                _ => SelectorIndex::Unknown as usize,
            }
        }
    };
}
